...
function Header() {
    ...
    return (
        ...
        animate={{opacity:1 }}
             className='sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full'>Generate Images  <img className='h-6' src={assets.star_group} alt="" /></motion.button>
            <motion.div 
             initial={{opacity:0 }}
        ...
        animate={{opacity:1 }}
            className='flex flex-wrap justify-center mt-16 gap-3'>
                {Array(6).fill().map((item, index) => (
                    ...
                    key={index} width={70} className='rounded hover:scale-105 transition-all cursor-pointer max-sm:w-10 ' src={index % 2 === 0 ? assets.sample_img_2 : assets.sample_img_1 } alt="" />
                ))}
